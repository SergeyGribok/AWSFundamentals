...
public class CustomerService : ICustomerService
{
    ...
    private readonly ISnsMessanger _snsMessanger;

    public CustomerService(ICustomerRepository customerRepository,
        IGitHubService gitHubService,
        ISnsMessanger snsMessanger)
    {
        _customerRepository = customerRepository;
        _gitHubService = gitHubService;
        _snsMessanger = snsMessanger;
    }
    ...
    public async Task<bool> CreateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _snsMessanger.PublishMessageAsync(customer.ToCustomerCreatedMessage());
        }
        return response;
    }
    ...
    public async Task<bool> UpdateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _snsMessanger.PublishMessageAsync(customer.ToCustomerUpdatedMessage());
        }
        return response;
    }
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        ...
        if (response)
        {
            await _snsMessanger.PublishMessageAsync(new CustomerDeleted { Id = id });
        }
        return response;
    }
